...
def profile(grid_size, **kwargs):
    grid_ping = torch.randint(0, 2, (grid_size, grid_size), dtype=torch.int8, device="cuda")
...
def test(**kwargs):
    grid_size = 16
    grid = torch.randint(0, 2, (grid_size, grid_size), dtype=torch.int8, device="cuda")

    # Reference implementation in pure torch
    kernel = torch.tensor([[[[1, 1, 1], [1, 0, 1], [1, 1, 1]]]], dtype=torch.float32, device="cuda")
    neighbors = torch.nn.functional.conv2d(grid.reshape((1, 1, grid_size, grid_size)).to(torch.float32), kernel, padding="same").reshape((grid_size, grid_size)).round().to(torch.int8)
...
def read_pattern(file_path, tensor):
    ...
    torch_pattern = torch.zeros((len(pattern), max_cols), dtype=torch.int8, device="cuda")
...
def show(grid_size, **kwargs):
    grid_ping = torch.randint(0, 2, (grid_size, grid_size), dtype=torch.int8)
